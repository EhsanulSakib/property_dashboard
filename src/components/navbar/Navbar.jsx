...
const Navbar = () => {
  ...
  return (
    <nav className='flex justify-between px-2 lg:px-4 py-2 items-center border-b border-neutralBorder'>
      ...
      {/* middle section */}
      <section className={`absolute lg:static top-[3.8rem] w-1/2 md:w-1/3 lg:w-auto h-screen lg:h-auto p-6 lg:p-0  bg-navbar lg:bg-transparent transform ${isOpen ? 'left-0 -translate-x-0' : '-left-[100%] -translate-x-full lg:translate-x-0'} transition-transform duration-300 z-50 ease-in-out shadow-lg lg:shadow-none`}>
        <ul className={`${isOpen ? 'flex' : 'hidden'} lg:flex flex-col lg:flex-row gap-4 font-semibold xl:text-lg`}>
          {
            navLinks.map((link) => (
              <li key={link.title} className={`relative hover:text-secondary cursor-pointer `}>
                <a href={link.url} className={`${pathname === link.url ? 'border-b border-neutralBorder lg:border-none text-2xl lg:text-lg xl:text-xl lg:text-primary' : ''}`}>{link.title}</a>
              </li>
            ))
          }
        </ul>
